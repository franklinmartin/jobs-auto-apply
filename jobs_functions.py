...
def assign_url(search_job_title, search_location_city, search_location_state, search_days):
...
""" def click(element, j):
    ...
    #element.click()
    driver.get( url + str(j) + "&company=&days=" + search_days)
    if "1-Click Apply" not in text:
        click(element, j)
        print(element.id + " clicked.\n")
    return find_buttons(j) """
...
search_days = sys.argv[3]

url = assign_url(search_job_title=search_job_title,
